...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # The occupancy grid is stored in self._map (x_val, y_val)
            step = min(self._x_res, self._y_res)
            lengths = np.arange(0, r, step)
            direction = np.array((np.cos(angle), np.sin(angle)))

            end_point = r * direction + np.array([sensor_x, sensor_y])
            x, y = self.PointToVoxel(*end_point)
            self._map[x, y] = min(self._map[x, y] + self._occupied_update, self._occupied_threshold)

            last_voxel_seen = x, y
            for length in reversed(lengths):
                point = direction * length + np.array([sensor_x, sensor_y])
                x, y = self.PointToVoxel(*point)
                if x == last_voxel_seen[0] and y == last_voxel_seen[1]:
                    continue
                # self._map[x, y] = max(self._map[x, y] - self._free_update, self._free_threshold)
                self._map[x, y] = max(self._map[x, y] + self._free_update, self._free_threshold)
                last_voxel_seen = (x, y)
